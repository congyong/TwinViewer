...
| 模块 | 能力 |
| --- | --- |
| 浏览 | 缩略图网格（懒加载、尺寸滑块）、递归子文件夹、格式过滤、名称/日期/大小排序、勾选、右键文件操作（复制/粘贴/新建文件夹/删除） |
...
自动校验目录扫描 / list-dirs / path-ancestors / read-file-buffer（含像素非零断言）/ 文件操作三件套 / twinview:// 协议链路，截屏保存 `smoke-home.png`，全部通过打印 `[SMOKE] 全部通过` 并退出。
...
│     ├─ CompareView.tsx                 A/B 对比（划变 / 并排 / 叠化 + 单格全屏）
│     ├─ CompareGrid.tsx                 多图网格对比（自动布局 / 同步独立 / 单格全屏）
