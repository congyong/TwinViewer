...
export function Toolbar() {
  ...
  return (
    <div data-chrome="toolbar" className="flex select-none flex-wrap items-center gap-x-3 gap-y-1.5 whitespace-nowrap border-b border-border bg-[var(--tv-panel)] px-3 py-2">
      ...
      {(viewMode === 'single' || viewMode === 'compare' || viewMode === 'grid') && (
        <>
          ...
          <Button
            size="sm"
            variant="ghost"
            className={`h-7 w-7 p-0 ${recPhase === 'recording' ? 'text-red-500' : ''}`}
