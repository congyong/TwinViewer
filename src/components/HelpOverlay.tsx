...
const SHORTCUTS: { keys: string; desc: string }[] = [
  ...
  { keys: '拖拽', desc: '平移图片' },
  { keys: 'I', desc: '显示 / 隐藏信息浮层（基本 + EXIF）' },
  { keys: 'H', desc: '显示 / 隐藏直方图（与工具栏开关同一状态，记住选择）' },
  { keys: '回车', desc: '浏览模式按勾选分流：勾选 1 张 → 该图单图；勾选 ≥2 张 → 对比选中（2 张 A/B 并排、≥3 张网格）；0 张 → 当前焦点图单图，无焦点无操作' },
  ...
  { keys: 'S', desc: '录制显示区：先弹配置对话框（格式 MP4/GIF + 画质档，记住上次选择）→「开始录制」后 3 秒倒计时（倒计时内再按取消）；录制中再按 = 立即停止并弹系统保存对话框选位置（不再重复询问格式/画质）' },
