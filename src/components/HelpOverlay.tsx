...
const SHORTCUTS: { keys: string; desc: string }[] = [
  ...
  { keys: 'F', desc: '单图 / 对比 / 网格 = 进入或退出视图级全屏（控件内）：对当前视图控件整体全屏，布局与图像数量不变' },
  { keys: 'Shift+F', desc: '物理全屏（隐藏浏览器 / 窗口边框）：同样作用于当前视图控件整体，自动带悬浮迷你条' },
  { keys: 'Alt（按住）', desc: '颜色探针：浮签显示原图坐标与 RGB；ALT+单击记录到侧栏取样列表' },
  { keys: '1', desc: '实际大小 100%（单图 / 对比）；网格中为激活第 1 格' },
  { keys: '1 – 9', desc: '网格模式：激活第 N 格' },
  { keys: '双击图片', desc: '单图：适应窗口 ↔ 100%；对比 / 网格：进入或退出视图级全屏（整个视图控件）' },
  { keys: 'Esc', desc: '物理全屏 → 视图级全屏 → 返回浏览模式（逐级退出）' },
