...
### 2.13 显示区录制（`src/lib/recorder.ts` + `RecordOverlay.tsx` + store rec*，第十一/十二轮）

- **状态机**（store，第十二轮改为**先配置后开录**）：`idle → configuring(开录前配置) → starting(3s 倒计时) → recording → stopping(3s 倒计时) → saving(系统保存对话框) → idle`；S 键/工具栏按钮 = `toggleRecord()`：idle→configuring、starting→取消回 idle、recording→stopping、stopping→取消回 recording（configuring/saving 中 S 无效）。配置对话框「开始录制」= `confirmRecConfig()`（进 starting 倒计时），「取消」/ Esc = `cancelRecConfig()`（Esc 在 useKeyboard 全局链最前端拦截 configuring）。倒计时 tick 用模块级 `recTickTimer`（store.setState({recCountdown}) 可被冒烟快进）；`recElapsedTimer` 驱动红点徽标计时；**10 分钟上限** `recMaxTimer` 自动 finalize。`setViewMode` 入口先 `stopRecordingCleanup()`（视图切换一律停止并释放 tracks，configuring 态同被覆盖）。
...
- **编码**：MediaRecorder mime 运行时探测（`video/mp4;codecs=avc1…` → `video/mp4` → `webm/vp9` → `webm`），**MP4 不可用落 WebM 且 UI 明示**；**码率按画质档于录制开始确定**（高8/中4/低2 Mbps；开录后改画质只影响 GIF 与下次录制——已知折衷）。**GIF**：录制时同步收集 10fps / 宽 360 帧（环形最近 30 秒 = 300 帧），保存时 gifenc 逐帧 `quantize+applyPalette`（画质档控制色数 256/128/64 与缩放 1/0.75/0.5）。
...
- **RecordOverlay**：挂在 App 显示区列（`relative` + absolute 叠层）：configuring 模态 `[data-rec-config]`（`[data-rec-format-video/gif]`、`[data-rec-quality]`、`[data-rec-start]`（autoFocus）、`[data-rec-cancel]`）、starting/stopping 中央胶囊 `[data-rec-pill]`、recording 右上红点徽标 `[data-rec-badge]`、saving 模态 `[data-rec-save]`（参数明示 `[data-rec-save-params]` + `[data-rec-save-ok]` 重试 / `[data-rec-discard]` 放弃）。
- IPC 新增 ×3：`get-window-source-id` / `rec-save-dialog` / `write-binary-file`（preload 同名透传，TwinviewBridge 类型同步）。
- 冒烟 a.13（不断言真实编码）：S 事件进 configuring + 配置对话框（格式/画质按钮、默认 video/medium）→ 选 GIF/低画质并断言 **localStorage 持久化** → 取消/Esc 回 idle → 再开记住上次选择 → 「开始录制」进 starting + 胶囊 → S 取消 → 确认快进倒计时 → **headless 下真实采集成功**（`captureOk:true`）→ 停止倒计时可取消 → 停止到 saving（自动保存被冒烟 IPC 模拟取消，留 saving）→ **保存参数与开录前一致**（store gif/low + 叠层参数文案 + 无重复询问按钮）→ 放弃回 idle；采集成败与 afterStop 仅报告不断言，saving 态无会话时用假 blob 兜底断言。
...
- 录制：`recPhase('idle'|'configuring'|'starting'|'recording'|'stopping'|'saving'), recCountdown, recElapsed, recMime, recBlob`（会话级）；`recFormat, recQuality`（**持久化**，开录前配置默认取上次选择）
