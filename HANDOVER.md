...
| `scan-directory` | `(dir: string, recursive: boolean)` | `{path,name,size,lastModified}[]` | 递归收集图片（扩展名白名单 11 种）；无权限项跳过；**递归跳过 `node_modules`/`.git`**（第十六轮，祖先链打开上级目录防卡死） |
...
- **树节点点击 = 回浏览**（第十轮）：Sidebar `TreeNode` 行点击在 `setCurrentPath(relPath)` 后追加 `setViewMode('browse')`——单图/对比/网格视图下点树即切回浏览网格（浏览中点击行为不变；`setViewMode` 自带 fullscreenCell 清理与缓存释放，Esc 链不回归）。冒烟 a.11 断言。**第十六轮起点击统一走 `openTreeNode(relPath)`**（见下条）。
- **树节点点击 = 增量扫描实时显示**（第十七轮重做，用户指定方案，`appStore.openTreeNode` + `scanDirIntoImages`）：点击任何树节点 = 切视野回浏览（第十轮语义）+ 对该目录**增量扫描**（recursive 开关决定是否含子树）——按归一化路径去重，**已在 images 中的跳过不更新、新文件追加**，勾选/槽位/打开根等状态全部保留；根外节点（祖先链等绝对 relPath）用绝对 currentPath（`scopeOk`/面包屑/navigateUp 原生支持）；根内绝对形折算相对路径。扫描中 `scanning=true` 显示右下角轻量提示 `[data-scanning]`（不整屏阻断网格，`scanInFlight` 计数防并发提前熄灭），失败 `showNotice` toast。Electron 走 `scanPath`（scan-directory IPC）；浏览器 FS Access 按 relPath 逐段 `getDirectoryHandle` 解析后 `listImages`（树中本无根外节点；webkitdirectory 回退打开时已全量在列表，无可增量）。
- **增量扫描触发点统一收口到 `setCurrentPath`**（第十七轮跟进，真实缺口修复）：此前仅树行点击（openTreeNode）触发扫描，**文件夹卡片双击 / 面包屑段点击 / navigateUp(Backspace) 直写 currentPath 不触发**——根外目录经这些入口进入时图片从未入列表 → 视图空白（用户真实路径复现：打开 `Downloads\0717\all` 点树中 `0616` 再双击其 sub 卡片，`v2_thumbs=0`；回退后开 recursive 也补不上，`v3_images=6/v3_thumbs=3`）。现在 `setCurrentPath` 在路径变化时统一 fire `scanDirIntoImages`（openTreeNode 只做根内/根外路径折算后委托）；`navigateUp` 改走 `setCurrentPath`；**`setRecursive` 切档后以新口径重扫当前目录**（浅扫遗漏的子树开递归立即补齐）。scopeOk 路径口径（双侧 normalizeSlashes + absDirOf）经变体矩阵验证本就一致，非根因。去重 append 在 set updater 内现算现滤，并发扫描不重复。
- ~~根外树节点点击 = 打开/扫描为新根~~（第十六轮方案，已被上方增量方案取代）：该方案整屏 loading 阻断网格、freshState 重置根/勾选/槽位，高位祖先（用户主目录/文档级）递归扫描巨量文件长时间空白——用户实测反馈"没解决问题"（复现证据：点击 workspace 祖先 `sawFullScreenLoading/rootReset/checkedWiped` 全 true）。`openPath` 整体重扫仅在显式「打开文件夹」/收藏/CLI 流程保留。
- **递归扫描跳过 `node_modules` / `.git`**（第十六轮保留，`electron/main.cjs scanDirectory` + `SCAN_SKIP_DIRS`）：树点击增量扫描与显式打开共用此规则，避免代码工作区类目录遍历数十万个文件（本仓库 node_modules `du` 超 120s 未能完成）。仅影响图片扫描，不影响 list-dirs 树结构展示。
- 冒烟 a.11b（第十七轮语义）：真实 DOM 点击祖先链 workspace 节点 → 轮询（≤60s）images 增量增长、**根不重置（rootKept）、勾选保留（checkedKept）**、currentPath 切为 workspace 绝对路径、网格 `[data-thumb]` >0；**重复点击去重不追加（deduped）**；根内相对/绝对形点击增量全去重（images 不变，绝对形折算 rel='sub'）；末尾恢复原始 images 数组。
- 冒烟 a.11c（根外兄弟目录固化，测试资产 `workspace/repro`：0717/all 3 张 + 0616 3 张 + 0616/sub 2 张）：打开 `repro/0717/all` → DOM 展开 repro 祖先 → 点击 0616 行 → 本层 3 张显示 + 勾选保留 + sub 文件夹卡片 → **双击 sub 卡片钻取 → 增量扫描补上 2 张（drillImages/drillThumbs）** → navigateUp 回退 0616 → setRecursive(true) 重扫后子树 5 张全显 → 重复点击去重 → openPath 复位 test-photos。
...
- **X 交换与左右键解耦（第十五轮修复）**：`swapSlots` 纯交换 A/B 后，若结果仍 `slotA===slotB!==null`（导航回退允许的同图态，如仅勾选 2 张走到 2/2），自动把交换后的 B 槽改为 `getNavList(s).find(e => e.id !== a)?.id`（集合内最近异图）——效果等价于「重复前一对的反序」，**任何导航状态下 X 都可见生效**；集合真只有 1 张时保持同图（状态仍交换 + activeSlot 翻转）。修复前同图下交换无视觉差异，用户感知为「X 失效」；全屏 L1/L2 走同一 action 同获修复；L3 双击切源只翻转 `fullscreenCell` 显示源、本就不改槽位内容，状态级翻转成立无需改。
- **showNotice 机制保留**：`notice` state + `showNotice(msg)`（3s 模块级计时器自动消失，App 根 `[data-notice]` 浮签）当前无调用方，留作后续一次性提示通道。N 下一对/下一组不变。

### 2.12 差值热图布局（diff，`src/lib/diffmap.ts` + `src/components/DiffPane.tsx`，第十一轮）

- `CompareLayout` 第 4 档 `'diff'`（持久化）；**D 键 = `toggleDiffLayout()`**：进入 diff 记录 `diffPrevLayout`（会话级），再按 D 还原；W/G `cycleCompareLayout` 仍只在 划变→并排→叠化 三档循环（diff 档时回划变）。工具栏 segmented 加「差值」档；**配置面板仅 diff 激活时显示**（`[data-diff-panel]`）：colormap 下拉 + 容差滑块/数值输入（0–128，默认 16）+ **均衡开关**（`[data-diff-equalize]`，第十五轮）+ **diff 最大值显示**（`[data-diff-max]`，DiffPane 重算后 `setDiffMax` 回报）；`diffColormap/diffTolerance/diffEqualize` 持久化到 settings，`diffMax` 会话级。
- **计算**（`computeDiffBitmap(bmpA, bmpB, tolerance, colormap, equalize=false, stats?)`）：公共采样网格 = **A 的自然尺寸**，B 经 canvas 平滑缩放到 A 尺寸（`toImageData`）；逐像素差值 = **三通道最大 abs diff**（max(|Δr|,|Δg|,|Δb|)，非欧氏距离——对通道级错位更敏感、计算更省）；**diff ≤ 容差 → 黑色**；否则默认**直方图均衡**（第十五轮）：>容差幅值统计 256-bin 直方图 → CDF 累积分布 `cdf[diff]/count` 重映射到 0–1 全区间 → 256 级 LUT（两图很接近时小差异拉伸可见；全黑 count=0 跳过避免除零；仅多一次直方图+CDF，开销可忽略），关闭回到 `(diff-容差)/(255-容差)` 线性归一；`stats.max` 出参回报全局 diff 最大值。LUT：inferno/viridis = matplotlib 锚点分段线性插值近似（视觉与原 LUT 一致），gray 线性，coolwarm = (59,76,192)→(221,221,221)→(180,4,38) 发散。数据源 = decode-cache bitmap（blob 解码，canvas 不污染）。
...
- 冒烟 a.12：UI 挂载（面板+canvas）→ 同图（A1 vs A1）中心像素全黑 → 异图非黑 → 容差滑块存在且拖动联动 store（native setter + input 事件）→ 面板仅 diff 布局可见（切并排隐藏、切回显示）→ 单元级（动态 `import('/src/lib/diffmap.ts')` + 合成 4×4 位图，d=100 恒定）：容差 16→64 灰度值单调下降、容差 100 全黑、gray 输出与 LUT 抽查均 R=G=B（消色差）、coolwarm LUT 中点近白且左端偏蓝/右端偏红、四种必需 colormap 单一来源列表齐全有序且均可切换生效、inferno≠viridis；**均衡（第十五轮）**：默认开（`DEFAULT_SETTINGS.diffEqualize===true`）+ 面板开关存在且点击联动 store + `[data-diff-max]` 存在；8 灰级差合成图（容差 4）均衡开映射 >0.7·255 高亮 / 关 <0.1·255、全同图跳过均衡仍全黑、`stats.max===8`。
...
- **开录前配置**（`recFormat/recQuality/recGifMode/recGifFrameSec` **持久化**到 settings，默认取上次选择）：格式 视频(MP4/WebM 按 `pickVideoMime()` 实时探测标注)/GIF + 画质 高/中/低；**GIF 格式下另有抓帧方式二选一**：`continuous` 连续采样 / `switch` **切换抓帧**（第十五轮起**新用户默认 switch**，见下条）+ switch 模式下**帧间时长（秒）数值输入**（`recGifFrameSec`，0.1–5，默认 0.5）；MP4 不可用与「GIF 10fps 最多 30 秒」等提示在配置对话框展示。**停止后不再询问格式/画质**：`finalizeCapture` 进 saving 即自动 `saveRecording()` 弹系统保存对话框选位置；用户取消路径选择/保存失败则留 saving，叠层提供「重试保存 / 放弃录制」。
...
- **GIF 切换抓帧（第十四轮；第十五轮默认化 + 固定帧间时长，`GifMode='continuous'|'switch'|'none'`）**：`startCapture(quality, gifMode)`——switch **不启动定时器**，帧尺寸 = 显示区设备像素**原尺寸不降采样**（仅 >`SWITCH_MAX_WIDTH=2560` 宽等比缩），环形上限 `min(SWITCH_MAX_FRAMES=60, GIF_MAX_BYTES/帧字节)`；事件源 = store 模块级 `useAppStore.subscribe`：仅 recording+gif+switch 时计算显示签名 `gifSig`（`currentId|slotA|slotB|gridIds|fullscreenCell`；wipe 分割线/缩放平移不算，L3 显示源切换算），变化后 **400ms 去抖**抓一帧（等切图解码上屏，期间连续变化只抓最后一次）；**首帧不自抓**（开录基线在 `beginCapture` 记录），全程零变化时 `finalizeCapture` 调 `grabFrame()` 补抓当前帧兜底；**C 键手动补帧**（useKeyboard，仅 recording+gif+switch 生效）。`encodeGif(quality, switchDelayMs?)`：switch 模式 per-frame delay **统一为固定帧间时长**（`recGifFrameSec`×1000，默认 500ms，**含末帧**；第十五轮起替代第十四轮的"真实时间戳间隔、末帧 1s"策略——播放节奏均匀可控；`gifTimes` 时间戳数组随之移除）；连续模式缺省均匀 delay 行为不变。帧内存：60 帧 1440p RGBA ≈885MB / 1080p ≈456MB，受 1.25GB 预算缩帧兜底。
...
- **RecordOverlay**：挂在 App 显示区列（`relative` + absolute 叠层）：configuring 模态 `[data-rec-config]`（`[data-rec-format-video/gif]`、`[data-rec-quality]`、GIF 格式下抓帧方式二选一 `[data-rec-gifmode="continuous"|"switch"]` + switch 模式帧间时长输入 `[data-rec-gifsec]`、`[data-rec-start]`（autoFocus）、`[data-rec-cancel]`）、starting 中央胶囊 `[data-rec-pill]`（「3S 后开始录制·再按 S 取消」）、recording 右上红点徽标 `[data-rec-badge]`（switch 模式追加 `[data-rec-frame-count]` 已抓帧数）、saving 模态 `[data-rec-save]`（参数明示 `[data-rec-save-params]` + `[data-rec-save-ok]` 重试 / `[data-rec-discard]` 放弃）。
- IPC 新增 ×3：`get-window-source-id` / `rec-save-dialog` / `write-binary-file`（preload 同名透传，TwinviewBridge 类型同步）。
- 冒烟 a.13（不断言真实编码）：S 事件进 configuring + 配置对话框（格式/画质按钮、默认 video/medium）→ 选 GIF/低画质并断言 **localStorage 持久化** → 取消/Esc 回 idle → 再开记住上次选择 → 「开始录制」进 starting + 胶囊 → S 取消 → 确认快进倒计时 → **headless 下真实采集成功**（`captureOk:true`）→ 录制 1.2s 攒帧 → **S 立即停止**（轮询相位序列断言 `noStopping` 无中间态且 `stopImmediate` 落 saving）→ 自动保存被冒烟 IPC 模拟取消留 saving → **保存参数与开录前一致**（store gif/low + 叠层参数文案 + 无重复询问按钮）→ 放弃回 idle；**GIF 单元**（动态 import gif-core：档位计划 fps/帧数/色数/抖动、尺寸上限、字节预算帧数、合成渐变帧编码出 GIF89a 合法头）；a.13b 高档实采 2.5s（渐变测试图）→ `encodeGif('high')` → 写盘 `gif-smoke-high.gif`（报告值，供人工目检）；采集成败与 afterStop/gifHigh 仅报告不断言，saving 态无会话时用假 blob 兜底断言；**a.13c 切换抓帧端到端**：默认断言（`DEFAULT_SETTINGS.recGifMode==='switch'` 且 `recGifFrameSec===0.5`）→ GIF/medium/switch + 帧间设 0.8s（配置框 `[data-rec-gifsec]` 可见且联动）开录（配置框出现 `[data-rec-gifmode="switch"]`）→ 首帧不自抓（=0）→ 两次槽位切换 + C 键各抓一帧（`recFrameCount=3`、徽标同步）→ 帧尺寸 = `<main>` 设备像素原尺寸（容差 2px，≤2560）→ S 立即停止 → `encodeGif` 解析 GCE 断言 delay ≥3 帧且**全 = 80**（固定帧间 0.8s×100）→ 放弃回 idle（复位回新默认 video/medium/switch/0.5s）。**注意：冒烟断言录制会话必须用 `window.__twinviewRecorder`**（store 在 appStore 底部暴露的同模块实例命名空间）——渲染进程里 `import('/src/lib/recorder.ts')` 与源码 `@/lib/recorder` 别名解析产物不同源，会拿到另一份 session 单例（a.13c 首跑 `无可用帧` 即此坑）。

### 2.14 对比选中入口与快捷分流（第十六轮）

- **「对比选中」固定并排**：`startCompareFromChecked` 的 2 张分支在进入 compare 时**重置 `compareLayout:'side'` + `diffPrevLayout:'side'` 并 `updateSettings` 持久化**——每次经工具栏「对比选中」/ 回车进入 A/B 对比都从并排开始，不沿用上次记住的布局；会话内 W/G 手动循环与布局持久化不受影响（仅入口动作重置）；≥3 张网格分支与 CLI `--layout` 路径**不重置**。
- **H 键 = 直方图开关**：useKeyboard 全局段（与 I 键并列），`toggleHisto()`——与工具栏 toggle 同一 store 状态，持久化不变；无键位冲突（帮助是 `?`）；HelpOverlay 表格与 README 快捷键表已同步（规约第 4 条）。
- **回车勾选分流**（useKeyboard 浏览分支，替代原「回车 = 焦点图单图」）：勾选 1 张 → `enterSingle(checked[0])`；勾选 ≥2 张 → `startCompareFromChecked()`（2 张 A/B 并排重置、≥3 张网格）；0 张 → 有 `currentId` 则 `enterSingle(currentId)`，否则无操作。单图/对比/网格分支回车保持无操作。
- 冒烟 a.8b（事件级，插在 a.8 后）：wipe/diff 布局下进对比选中均重置 side（`presetWipe/sideReset/sideResetFromDiff`）+ ≥3 张进网格布局不重置（`gridNoReset`）+ H 大小写事件往返（`hToggled/hToggledBack`）+ 回车四路分流（`enterOne/enterTwo(含 side 重置)/enterThree/enterZero`）；**复位时经 `setCompareLayout('side')` action 复位**——冒烟 userData 未隔离，localStorage 布局设置会带入下次运行，直接 setState 会造成跨轮漂移。
...
- 差值：`diffColormap, diffTolerance, diffEqualize（持久化）, diffPrevLayout, diffMax（会话级）`
- 录制：`recPhase('idle'|'configuring'|'starting'|'recording'|'saving'), recCountdown, recElapsed, recMime, recBlob, recFrameCount`（会话级）；`recFormat, recQuality, recGifMode, recGifFrameSec`（**持久化**，开录前配置默认取上次选择）
...
- 冒烟自检：`npm run build && TWINVIEW_SMOKE=1 NODE_ENV=production ./node_modules/electron/dist/electron.exe .`（断言点：10 图扫描、list-dirs、path-ancestors、read-file-buffer 像素非零、文件操作三件套、打开对话框 IPC（special-dirs/browse-dir/dir-image-preview 递归 count=10+4 张 + shallow 本层 count=8/dirs 含 sub/缩略图无子目录）、**UI 自动化：openPath 后断言递归关 8 张 ↔ 开 10 张、子文件夹卡片 `[data-folder]`、面包屑 `nav`、列表模式行数、`setCurrentPath('sub')` 面包屑段数与 `navigateUp()` 回根、主题亮/暗 class 切换、打开文件夹对话框渲染（含 sub 子目录按钮）**、**CLI 注入（send cli-open：folder+file 定位选中 sub 内文件；--compare 断言 viewMode/slotA/slotB/layout/theme/recursive flag）**、**真全屏布局（physicalFullscreen 状态级模拟：`[data-chrome]`/aside 全隐藏 → 恢复）+ 槽位导航（仅勾选占满时回退同图+无 notice、全部档步进跳过另一槽、激活侧切换步进、swap 回归、网格跳过占据格+网格回退同图）+ X 交换解耦（第十五轮：同图后 X 自动换 B 槽可见生效 `xDupSwap`、连按两次回原图 `xDupSwap2`、全部档 `xAllSwap`、全屏 L1/L2 生效 `fsXSwap`、L3 双击切源状态级翻转 `fsSrcFlip/fsSrcFlip2`）+ a.8b 对比选中固定并排/H/回车分流（第十六轮：wipe/diff 进入重置 side、≥3 网格不重置、H 大小写往返、回车 1 张单图/2 张并排 A/B/≥3 网格/0 张焦点图）+ a.11b 树点击增量扫描（第十七轮：DOM 点击 workspace 祖先 → images 增量追加、根不重置、勾选保留、绝对路径视野、网格缩略图 >0、重复点击去重、根内相对/绝对形全去重）+ a.11c 根外兄弟目录钻取（repro 镜像：点击 0616 本层即显、双击 sub 卡片钻取增量补齐、navigateUp/递归切档重扫、去重复位）+ 视图级物理全屏 a.9（Shift+F 直进：对比 2 pane / 网格 4 pane 布局不变 + `[data-minibar]` + chrome 全卸载，退出恢复）+ 双击三层链 a.10（事件级 dblclick 进 L1 单格全屏；action 触发物理请求 + 状态级模拟 L2；L3 对比 A↔B 槽位内容不变、网格 '0'→'1'→'2' 格组不变；退出 chrome 恢复）+ 树点击回浏览 a.11（对比下模拟树节点点击 → browse + currentPath 正确；浏览中不变；Esc 回归）+ 差值热图 a.12（面板仅 diff 可见+滑块联动、canvas 挂载、同图全黑、异图非黑、单元级合成位图验容差、四种 colormap 齐全有序可切换、coolwarm 中点近白、gray 消色差、**均衡默认开+开关联动+8 灰级差开高亮/关暗、全同图全黑**）+ 录制状态机 a.13（S→配置对话框（GIF/低画质+持久化断言）→确认进倒计时→采集→**S 立即停止（无 stopping）**→saving 自动保存（冒烟模拟取消）→参数一致→放弃回 idle；GIF 单元 gif-core 档位/编码合法；a.13b 高档实采目检样本；**a.13c 切换抓帧：默认 switch/0.5s→设 0.8s→开录首帧不自抓→两次槽位切换+C=3 帧+徽标同步→帧尺寸=显示区设备像素原尺寸→立即停止→GCE delay 全 80（固定帧间）→放弃回 idle**，采集成败仅报告）**、`<img>` 协议探测；输出 `[SMOKE]`，失败 `[SMOKE-FAIL]` 退出码 1）；**冒烟基础设施（第十五轮稳定性修复，勿回退）**：`BrowserWindow` webPreferences 在 `TWINVIEW_SMOKE` 下设 `backgroundThrottling:false`（窗口被遮挡时 Chromium 节流 setTimeout 会致各 eval 随机卡死）、killer 超时 120s、a.13/a.13c 倒计时等待改轮询相位（负载下 tick+采集启动可超固定 wait）；
