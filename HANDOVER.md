...
- **树节点点击 = 增量扫描实时显示**（第十七轮重做，用户指定方案，`appStore.openTreeNode` + `scanDirIntoImages`）：点击任何树节点 = 切视野回浏览（第十轮语义）+ 对该目录**增量扫描**（recursive 开关决定是否含子树）——按归一化路径去重，**已在 images 中的跳过不更新、新文件追加**，勾选/槽位/打开根等状态全部保留；根外节点（祖先链等绝对 relPath）用绝对 currentPath（`scopeOk`/面包屑/navigateUp 原生支持）；根内绝对形折算相对路径。扫描中 `scanning=true` 显示右下角轻量提示 `[data-scanning]`（不整屏阻断网格），失败 `showNotice` toast。Electron 走 `scanPath`（scan-directory IPC）；浏览器 FS Access 按 relPath 逐段 `getDirectoryHandle` 解析后 `listImages`（树中本无根外节点；webkitdirectory 回退打开时已全量在列表，无可增量）。
...
- 冒烟 a.11b（第十七轮语义）：真实 DOM 点击祖先链 workspace 节点 → 轮询（≤60s）images 10→20 增量增长、**根不重置（rootKept）、勾选保留（checkedKept）**、currentPath 切为 workspace 绝对路径、网格 `[data-thumb]` >0；**重复点击去重不追加（deduped）**；根内相对/绝对形点击增量全去重（images 不变，绝对形折算 rel='sub'）；末尾恢复原始 images 数组。
