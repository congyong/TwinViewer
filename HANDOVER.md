...
- **开录前配置**（`recFormat/recQuality/recGifMode` **持久化**到 settings，默认取上次选择）：格式 视频(MP4/WebM 按 `pickVideoMime()` 实时探测标注)/GIF + 画质 高/中/低；**GIF 格式下另有抓帧方式二选一**：`continuous` 连续采样（默认，按画质档定时抓帧）/ `switch` **切换抓帧**（第十四轮，见编码条）；MP4 不可用与「GIF 10fps 最多 30 秒」等提示在配置对话框展示。**停止后不再询问格式/画质**：`finalizeCapture` 进 saving 即自动 `saveRecording()` 弹系统保存对话框选位置；用户取消路径选择/保存失败则留 saving，叠层提供「重试保存 / 放弃录制」。
...
- **GIF 切换抓帧（第十四轮，`GifMode='continuous'|'switch'|'none'`）**：`startCapture(quality, gifMode)`——switch **不启动定时器**，帧尺寸 = 显示区设备像素**原尺寸不降采样**（仅 >`SWITCH_MAX_WIDTH=2560` 宽等比缩），环形上限 `min(SWITCH_MAX_FRAMES=60, GIF_MAX_BYTES/帧字节)`；事件源 = store 模块级 `useAppStore.subscribe`：仅 recording+gif+switch 时计算显示签名 `gifSig`（`currentId|slotA|slotB|gridIds|fullscreenCell`；wipe 分割线/缩放平移不算，L3 显示源切换算），变化后 **400ms 去抖**抓一帧（等切图解码上屏，期间连续变化只抓最后一次）；**首帧不自抓**（开录基线在 `beginCapture` 记录），全程零变化时 `finalizeCapture` 调 `grabFrame()` 补抓当前帧兜底；**C 键手动补帧**（useKeyboard，仅 recording+gif+switch 生效）。`encodeGif` 对 switch 模式用 `encodeGifFrames(..., delays)` 逐帧 delay = 真实时间戳差（下限 20ms 防播放器异常，**末帧固定 1000ms**）；连续模式缺省均匀 delay 行为不变。帧内存：60 帧 1440p RGBA ≈885MB / 1080p ≈456MB，受 1.25GB 预算缩帧兜底。
...
- 录制：`recPhase('idle'|'configuring'|'starting'|'recording'|'saving'), recCountdown, recElapsed, recMime, recBlob, recFrameCount`（会话级）；`recFormat, recQuality, recGifMode`（**持久化**，开录前配置默认取上次选择）
