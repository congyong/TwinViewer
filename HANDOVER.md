...
- **根外树节点点击 = 打开/扫描为新根**（第十六轮修复，`appStore.openTreeNode`）：修复前 TreeNode 对祖先链节点也只 `setCurrentPath` 做视野过滤——打开文件夹只递归扫描了所选根子树，祖先目录图片从未进入 `images`，非递归下一张看不到、递归下也仅原子树图片（复现证据：打开 test-photos 后点 workspace 祖先，递归关 0 张 / 递归开仅 10 张）。现在：relPath 为绝对路径且**不在 `dir.dirPath` 根内** → 复用 `openPath` 扫描打开为新根（重扫非增量：freshState 清勾选/槽位/树缓存，祖先链与树数据随之重建）；**根内**（含根本身的绝对形、祖先链展开后子节点的绝对路径）→ 折算相对路径走原快速视野过滤**不重扫**；浏览器模式 `dir.dirPath` 为空且树中无绝对路径节点，天然不受影响。
- **递归扫描跳过 `node_modules` / `.git`**（第十六轮，`electron/main.cjs scanDirectory` + `SCAN_SKIP_DIRS`）：祖先链点击会把上级目录（典型如代码工作区）作为新根递归扫描，不跳过依赖/版本库目录会遍历数十万个文件长时间无响应（本仓库 node_modules `du` 超 120s 未能完成）。仅影响图片扫描，不影响 list-dirs 树结构展示。
- 冒烟 a.11b：真实 DOM 点击祖先链 workspace 节点 → 轮询（≤60s）扫描完成且 `images` 10→20 增长、新根 = workspace、网格 `[data-thumb]` >0、树子目录同时含 twinview/test-photos；范围内相对路径与根内绝对形点击均快速过滤不重扫（images 不变、无 loading）；末尾 `openPath` 复位原测试根。
