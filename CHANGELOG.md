...
- **GIF 切换抓帧模式**：开录前配置对话框在 GIF 格式下新增抓帧方式二选一（连续采样 / **切换抓帧**，选择持久化）；切换抓帧 = 当前显示图像变化（单图/对比槽位/网格/全屏格切换，400ms 去抖等解码上屏）才抓帧，**全分辨率不降采样**（显示区设备像素原尺寸，仅 >2560 宽封顶），上限 60 帧；录制中按 **C** 手动补帧，红点徽标实时显示已抓帧数；全程无变化时停止自动补抓当前帧兜底
- **GIF 切换抓帧帧间时长可配**：配置对话框加「帧间时长（秒）」数值输入（0.1–5，默认 0.5，持久化）；编码时每帧 delay 统一用该值（含末帧）
- **差值热图直方图均衡**：容差过滤后对剩余 diff 幅值按 CDF 累积分布重映射到全区间再过 colormap——两张图很接近时（如仅差几灰级）小差异从近全黑拉伸为清晰可见；配置面板加「均衡」开关（默认开，持久化，关闭回到容差–255 线性归一）与 diff 最大值实时显示；全黑（无超容差像素）自动跳过均衡；开销仅一次 256-bin 直方图+CDF

### 变更

- **GIF 抓帧方式默认改为「切换抓帧」**（仅新用户默认值，已持久化的选择不受影响）
- **切换抓帧帧 delay 策略调整**：由「真实抓帧时间戳间隔（末帧固定 1 秒）」改为「固定帧间时长（默认 0.5s，可配）」——播放节奏均匀可控，快速连切不再产生过长停顿
