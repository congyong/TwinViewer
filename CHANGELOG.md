...
- **GIF 切换抓帧模式**：开录前配置对话框在 GIF 格式下新增抓帧方式二选一（连续采样 / **切换抓帧**，选择持久化）；切换抓帧 = 当前显示图像变化（单图/对比槽位/网格/全屏格切换，400ms 去抖等解码上屏）才抓帧，**全分辨率不降采样**（显示区设备像素原尺寸，仅 >2560 宽封顶），帧时长按真实时间戳间隔（末帧固定 1 秒），上限 60 帧；录制中按 **C** 手动补帧，红点徽标实时显示已抓帧数；全程无变化时停止自动补抓当前帧兜底
